...
template<typename T>
class BuildPreconditioner_BlackBlock_Grid3 {
...
public : 
	...
	BuildPreconditioner_BlackBlock_Grid3
	...
	void buildPreconditioner(const Block block) const {
		...
		for(i=block.begin[0];i<block.end[0]-1;++i) {
			for(j=block.begin[1];j<block.end[1]-1;++j) {
				for(k=block.begin[2];k<block.end[2]-1;++k) {
					if(!isBoundary(i,j,k)) {
						d = poisson(i,j,k,0) 
			...
			for(k=block.begin[2];k<block.end[2]-1;++k) {
				if(!isBoundary(i,j,k)) {
					d = poisson(i,j,k,0) 
						...
						- SQ( poisson(i,j-1,k,2)*preconditioner(i,j-1,k) )
						- SQ( poisson(i,j,k-1,3)*preconditioner(i,j,k-1) )
						- SQ( poisson(i,j,k,  1)*preconditioner(i+1,j,k) );
					preconditioner(i,j,k) = 1.0f / sqrt((float)::abs(d)+0.001f);
				}
			}
		}
			...
			for(k=block.begin[2];k<block.end[2]-1;++k) {
				if(!isBoundary(i,j,k)) {
					d = poisson(i,j,k,0) 
						...
						- SQ( poisson(i,j-1,k,2)*preconditioner(i,j-1,k) )
						- SQ( poisson(i,j,k-1,3)*preconditioner(i,j,k-1) )
						- SQ( poisson(i,j,k  ,2)*preconditioner(i,j+1,k) );
					preconditioner(i,j,k) = 1.0f / sqrt((float)::abs(d)+0.001f);
				}
			}
		}
			...
			for(j=block.begin[1];j<block.end[1]-1;++j) {
				if(!isBoundary(i,j,k)) {
					d = poisson(i,j,k,0) 
						...
						- SQ( poisson(i,j-1,k,2)*preconditioner(i,j-1,k) )
						- SQ( poisson(i,j,k-1,3)*preconditioner(i,j,k-1) )
						- SQ( poisson(i,j,k  ,3)*preconditioner(i,j,k+1) );
					preconditioner(i,j,k) = 1.0f / sqrt((float)::abs(d)+0.001f);
				}
			}
		}
...
template<typename T>
class ForwardSubstitution_BlackBlock_Grid3 {
	typedef typename Grid3_Preconditioner_BlockRedBlackICCG<T>::Block Block;
	typedef typename Grid3_Preconditioner_BlockRedBlackICCG<T>::index_type index_type;
...
public : 
	...
	ForwardSubstitution_BlackBlock_Grid3
			...
			for(k=block.begin[2];k<block.end[2]-1;++k) {
				if(!isBoundary[IS(i,j,k)]) {
					d = x[IS(i,j,k)] 
						...
						- poisson[4*IS(i,j-1,k)+2]*preconditioner[IS(i,j-1,k)]*m[IS(i,j-1,k)]
						- poisson[4*IS(i,j,k-1)+3]*preconditioner[IS(i,j,k-1)]*m[IS(i,j,k-1)]
						- poisson[4*IS(i,j,k  )+1]*preconditioner[IS(i+1,j,k)]*m[IS(i+1,j,k)];
					m[IS(i,j,k)] = preconditioner[IS(i,j,k)]*d;
				}
			}
		}
			...
			for(k=block.begin[2];k<block.end[2]-1;++k) {
				if(!isBoundary[IS(i,j,k)]) {
					d = x[IS(i,j,k)] 
						...
						- poisson[4*IS(i,j-1,k)+2]*preconditioner[IS(i,j-1,k)]*m[IS(i,j-1,k)]
						- poisson[4*IS(i,j,k-1)+3]*preconditioner[IS(i,j,k-1)]*m[IS(i,j,k-1)]
						- poisson[4*IS(i,j,k  )+2]*preconditioner[IS(i,j+1,k)]*m[IS(i,j+1,k)];
					m[IS(i,j,k)] = preconditioner[IS(i,j,k)]*d;
				}
			}
		}
			...
			for(j=block.begin[1];j<block.end[1]-1;++j) {
				if(!isBoundary[IS(i,j,k)]) {
					d = x[IS(i,j,k)] 
						...
						- poisson[4*IS(i,j-1,k)+2]*preconditioner[IS(i,j-1,k)]*m[IS(i,j-1,k)]
						- poisson[4*IS(i,j,k-1)+3]*preconditioner[IS(i,j,k-1)]*m[IS(i,j,k-1)]
						- poisson[4*IS(i,j,k  )+3]*preconditioner[IS(i,j,k+1)]*m[IS(i,j,k+1)];
					m[IS(i,j,k)] = preconditioner[IS(i,j,k)]*d;
				}
			}
		}
...
template<typename T>
class BackwardSubstitution_BlackBlock_Grid3 {
	typedef typename Grid3_Preconditioner_BlockRedBlackICCG<T>::Block Block;
	typedef typename Grid3_Preconditioner_BlockRedBlackICCG<T>::index_type index_type;
...
public : 
	...
	BackwardSubstitution_BlackBlock_Grid3
	(const Grid3<bool,T>& _isBdry, const Grid3x4<T>& _poi, const Grid3<T,T>& _pre, 
...
#define IS(i,j,k) ((k) + (nZ) * ((j) + (nY) * (i)))
		...
		i = block.end[0]-1;
		j = block.end[1]-1;
		for(k=block.end[2]-2;k>=block.begin[2];--k) {
			if(!isBoundary[IS(i,j,k)]) {
				d = m[IS(i,j,k)] 

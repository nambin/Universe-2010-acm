...
template<typename T>
class BuildPreconditioner_RedBlock_Grid2 {
...
public : 
	...
	BuildPreconditioner_RedBlock_Grid2
...
template<typename T>
class ForwardSubstitution_RedBlock_Grid2 {
	typedef typename Grid2_Preconditioner_BlockRedBlackICCG<T>::Block Block;
...
public : 
	...
	ForwardSubstitution_RedBlock_Grid2
...
template<typename T>
class BackwardSubstitution_RedBlock_Grid2 {
	typedef typename Grid2_Preconditioner_BlockRedBlackICCG<T>::Block Block;
...
public : 
	...
	BackwardSubstitution_RedBlock_Grid2
	(const Grid2<bool,T>& _isBdry, const Grid2x3<T>& _poi, const Grid2<T,T>& _pre, 
		...
		for(j=block.end[1]-1;j>block.begin[1];--j) {
			if( !isBoundary(i,j) )	{
				d = m(i,j) 
					- poisson(i,j,1)*preconditioner(i,j)*y(i+1,j)
					- poisson(i,j,2)*preconditioner(i,j)*y(i,j+1)
					- poisson(i-1,j,1)*preconditioner(i,j)*y(i-1,j);
				y(i,j) = preconditioner(i,j)*d;
			}
		}
		...
		for(i=block.end[0]-1;i>block.begin[0];--i) {
			if( !isBoundary(i,j) )	{
				d = m(i,j) 
					- poisson(i,j,1)*preconditioner(i,j)*y(i+1,j)
					- poisson(i,j,2)*preconditioner(i,j)*y(i,j+1)
					- poisson(i,j-1,2)*preconditioner(i,j)*y(i,j-1);
				y(i,j) = preconditioner(i,j)*d;
			}
		}

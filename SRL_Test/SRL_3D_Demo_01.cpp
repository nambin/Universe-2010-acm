...
void SRL_FluidSimulation_3D_Demo_01(Scene* scene) {
#ifdef USE_OPENMP	
	...
	bunny.set_scale(1.18);
	bunny.set_rigid_transformation(arithmetic::translate(Vector3<real>(0.46*Lx,0.75*Ly,0.5*Lz)));
	jointfield.add_distancefields(&bunny);

	WaterDropDistanceField3D<real> water(Vector3<real>(10000.0), 0.0, 0.065*Ly);
	...
	const int num_pole = 4;
	const T r_space_end = 0.17, r_radius = 0.035, r_space = (1.0 - (r_space_end * 2) - (r_radius * 2 * num_pole)) / float(num_pole - 1);
	const T r_pos_y = 0.24 + 0.035, r_pos_y2 = 0.38 + 0.035, r_length = 0.7;

	const T space_end = int(r_space_end * nX) * h, radius = int(r_radius * nX) * h, space = int((Lx - (space_end * 2) - (radius * 2 * num_pole)) / float(num_pole - 1) / h) * h;
	const T pos_y = int(r_pos_y * nY) * h, pos_y2 = int(r_pos_y2 * nY) * h, length = int(r_length * nZ) * h;

	Vector3<T> pole_center(space_end + radius, pos_y, Lz*0.5); 

	// Option 1
	for(int i=0;i<4;++i) {	
		log_info << "Cylinder " << i << " _ " << "r _ " << radius << " , length _ " << length << std::endl;
		log_info << "Translate _ " << pole_center << " Rotation x _ 90" << std::endl << std::endl;

		Cylinder<T> pole(radius, length);
		pole.set_rigid_transformation(arithmetic::translate(pole_center) * arithmetic::rotateX(real(90.0)));
		pole_center[0] += (space + radius * 2.0);		

		object3D->push_DynamicObject(StaticRigidBody3D<T>(pole));
	}	

	pole_center[0] = space_end + radius + (space * 0.5 + radius);
	pole_center[1] = pos_y2;
	pole_center[2] = Lz * 0.5;
	...
	//const T r_space_end = 0.20, r_radius = 0.042, r_space = (1.0 - (r_space_end * 2) - (r_radius * 2 * num_pole)) / float(num_pole - 1);
	//const T r_pos_y = 0.24, r_pos_y2 = 0.38, r_length = 0.7;

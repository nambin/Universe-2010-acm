#include <stdafx.h>
#include <Illumination/IlluminationHeader/PhotonMap/PhotonMap.h>

using namespace illuminationspace;

PhotonMap::PhotonMap(int max_phot)	{
	stored_photons = 0;
	prev_scale = 1;
	...
	bbox_min[0] = bbox_min[1] = bbox_min[2] = 1.0e+8;
	bbox_max[0] = bbox_max[1] = bbox_max[2] = -1.0e+8;
...
void PhotonMap::locate_photons(NearestPhotons* np, const real pos[3], const real max_dist, const int nPhotons) const	{
...
// photons once they have been emitted from the light source. scale = 1/(#emitted Photons)
...
// with all photons below the median in the lower half and all photons above the median in the upper half.
// The comparison criteria is the axis (indicated by the axis parameter)
// (inspired by routine in "Algorithms in C++ by Sedgewich)
